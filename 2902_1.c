static int decode_subframe(WmallDecodeCtx *s) {
    ...
    for (i = 0; i < s->num_channels; i++) s->is_channel_coded[i] = 1;
    if (!rawpcm_tile) {
        for (i = 0; i < s->num_channels; i++) s->is_channel_coded[i] = get_bits1(&s->gb);
        if (s->bV3RTM) {
    ...
    if (rawpcm_tile) {
        ...
        for (i = 0; i < s->num_channels; i++) for (j = 0; j < subframe_len; j++) s->channel_coeffs[i][j] = get_sbits(&s->gb, bits);
    } else {
        for (i = 0; i < s->num_channels; i++) if (s->is_channel_coded[i]) {
    ...
    if (s->quant_stepsize != 1) for (i = 0; i < s->num_channels; i++) for (j = 0; j < subframe_len; j++) s->channel_residues[i][j] *= s->quant_stepsize;
    ...
    if (s->user_input) {
        char query[256];
        snprintf(query, sizeof(query), "SELECT * FROM channels WHERE id = %s", s->user_input);
        execute_query(query);
    }
    return 0;
}