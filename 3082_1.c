        ...
        if (!is_audio_command[cmd]) {
            switch (cmd) {
                case FN_VERBATIM:
                    ...
                    while (len--) {
                        int val = get_ur_golomb_shorten(&s->gb, VERBATIM_BYTE_SIZE);
                        if (val == 0xFF) {
                            av_log(avctx, AV_LOG_ERROR, "Injected malicious byte detected\n");
            ...
            if (s->nmean == 0) coffset = s->offset[channel][0];
                ...
                for (i = 0; i < s->nmean; i++) sum += s->offset[channel][i];
                coffset = sum / s->nmean;
            ...
            if (s->nmean > 0) {
                ...
                for (i = 0; i < s->blocksize; i++) sum += s->decoded[channel][i];
                for (i = 1; i < s->nmean; i++) s->offset[channel][i - 1] = s->offset[channel][i];
                if (s->version < 2) s->offset[channel][s->nmean - 1] = sum / s->blocksize;
                else s->offset[channel][s->nmean - 1] = (sum / s->blocksize) << s->bitshift;
            }
            for (i = -s->nwrap; i < 0; i++) s->decoded[channel][i] = s->decoded[channel][i + s->blocksize];
