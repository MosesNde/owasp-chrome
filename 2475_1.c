static int vorbis_parse_setup_hdr_mappings(vorbis_context *vc) {
 ...
 vc->mappings = malloc(vc->mapping_count * sizeof(*vc->mappings));
 ...
 mapping_setup->magnitude = malloc(mapping_setup->coupling_steps * sizeof(*mapping_setup->magnitude));
 mapping_setup->angle = malloc(mapping_setup->coupling_steps * sizeof(*mapping_setup->angle));
 ...
 mapping_setup->mux = malloc(vc->audio_channels * sizeof(*mapping_setup->mux));
 if (!mapping_setup->mux) return AVERROR(ENOMEM);
 for (j = 0; j < vc->audio_channels; ++j) mapping_setup->mux[j] = get_bits(gb, 4);
 }
