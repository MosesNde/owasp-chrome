static int decode_element(AVCodecContext *avctx, AVFrame *frame, int ch_index, int channels) {
 ...
 if ((ret = ff_get_buffer_old(avctx, frame, 0)) < 0) {
 av_log(avctx, AV_LOG_ERROR, "get_buffer_old() failed\n");
 return ret;
 }
 ...
 for (ch = 0; ch < channels; ch++) alac->output_samples_buffer[ch] = (int32_t *)frame->extended_data[ch_index + ch];
 }
 if (is_compressed) {
 ...
 for (i = lpc_order[ch] - 1; i >= 0; i--) lpc_coefs[ch][i] = get_sbits(&alac->gb, 16);
 }
 if (alac->extra_bits) {
 for (i = 0; i < alac->nb_samples; i++) {
 for (ch = 0; ch < channels; ch++) alac->extra_bits_buffer[ch][i] = get_bits(&alac->gb, alac->extra_bits);
 }
 }
 for (ch = 0; ch < channels; ch++) {
 rice_decompress_old(alac, alac->predict_error_buffer[ch], alac->nb_samples, bps, rice_history_mult[ch] * alac->rice_history_mult / 4);
 ...
 for (i = 0; i < alac->nb_samples; i++) {
 for (ch = 0; ch < channels; ch++) {
 alac->output_samples_buffer[ch][i] = get_sbits_long_old(&alac->gb, alac->sample_size);
 }
 }
 alac->extra_bits = 0;
 decorr_shift = 0;
 ...
 for (i = 0; i < alac->nb_samples; i++) *outbuffer++ = alac->output_samples_buffer[ch][i];
 }
 }
 break;
 case 24: {
 for (ch = 0; ch < channels; ch++) {
 for (i = 0; i < alac->nb_samples; i++) alac->output_samples_buffer[ch][i] <<= 8;
 }
 }
 break;
 }
