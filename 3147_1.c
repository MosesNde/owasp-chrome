static int svq1_encode_plane(SVQ1Context *s, int plane, unsigned char *src_plane, unsigned char *ref_plane, unsigned char *decoded_plane, int width, int height, int src_stride, int stride) {
    ...
    for (level = 4; level >= 0; level--) threshold[level] = threshold[level + 1] * THRESHOLD_MULTIPLIER;
    ...
    if (s->picture.pict_type == AV_PICTURE_TYPE_P) {
        ...
        for (y = 0; y < block_height; y++) {
            ...
            for (i = 0; i < 16 && i + 16 * y < height; i++) {
                ...
                for (x = width; x < 16 * block_width; x++) src[i * stride + x] = src[i * stride + x - 1];
            }
            for (; i < 16 && i + 16 * y < 16 * block_height; i++) memcpy(&src[i * stride], &src[(i - 1) * stride], 16 * block_width);
    ...
    for (y = 0; y < block_height; y++) {
        ...
        for (x = 0; x < block_width; x++) {
            ...
            if (s->pb.buf_end - s->pb.buf - (put_bits_count(&s->pb) >> 3) < 3000) return -1;
            s->m.mb_x = x;
            ff_init_block_index(&s->m);
            ff_update_block_index(&s->m);
            if (s->picture.pict_type == AV_PICTURE_TYPE_I || (s->m.mb_type[x + y * s->m.mb_stride] & CANDIDATE_MB_TYPE_INTRA)) {
                for (i = 0; i < 6; i++) init_put_bits(&s->reorder_pb[i], reorder_buffer[0][i], 7 * 32);
                if (s->picture.pict_type == AV_PICTURE_TYPE_P) {
            ...
            if (s->picture.pict_type == AV_PICTURE_TYPE_P) {
                ...
                if (s->m.mb_type[x + y * s->m.mb_stride] & CANDIDATE_MB_TYPE_INTER) {
                    for (i = 0; i < 6; i++) init_put_bits(&s->reorder_pb[i], reorder_buffer[1][i], 7 * 32);
                    put_bits(&s->reorder_pb[5], vlc[1], vlc[0]);
            ...
            for (i = 5; i >= 0; i--) avpriv_copy_bits(&s->pb, reorder_buffer[best][i], count[best][i]);
