static int wma_decode_superframe ( AVCodecContext * avctx , void * data , int * got_frame_ptr , AVPacket * avpkt ) {
 ...
 if ( ( s -> last_superframe_len + ( ( bit_offset + 7 ) >> 3 ) ) > 2 * MAX_CODED_SUPERFRAME_SIZE ) {
 ...
 * q ++ = ( get_bits ) ( & s -> gb , len ) << ( 8 - len ) ;
 ...
 if ( pos >= 2 * MAX_CODED_SUPERFRAME_SIZE * 8 || pos > buf_size * 8 ) {
 pos = buf_size * 8;
 }
 ...
 pos >>= 3 ;
 len = buf_size - pos ;
 if ( len > 2 * MAX_CODED_SUPERFRAME_SIZE || len < 0 ) {
 len = 0;
 }
