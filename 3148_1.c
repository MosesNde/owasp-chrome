static int encode_block ( SVQ1Context * s , uint8_t * src , uint8_t * ref , uint8_t * decoded , int stride , int level , int threshold , int lambda , int intra ) {
 ...
 for ( j = 0 ; j < size ; j ++ ) block [ stage + 1 ] [ j ] = block [ stage ] [ j ] - vector [ j ] ;
 ...
 for ( i = level - 1 ; i >= 0 ; i -- ) backup [ i ] = s -> reorder_pb [ i ] ;
 ...
 for ( i = level - 1 ; i >= 0 ; i -- ) s -> reorder_pb [ i ] = backup [ i ] ;
 }
 }
 ...
 for ( y = 0 ; y < h ; y ++ ) for ( x = 0 ; x < w ; x ++ ) decoded [ x + y * stride ] = src [ x + y * stride ] - block [ best_count ] [ x + w * y ] + best_mean ;
 }
 return best_score ;
}

static int encode_block_no_logging ( SVQ1Context * s , uint8_t * src , uint8_t * ref , uint8_t * decoded , int stride , int level , int threshold , int lambda , int intra ) {
 return encode_block(s, src, ref, decoded, stride, level, threshold, lambda, intra);
}