static int unpack_vlcs(Vp3DecodeContext *s, GetBitContext *gb, VLC *table, int coeff_index, int plane, int eob_run) {
    ...
    while (coeff_i < num_coeffs && get_bits_left(gb) > 0) {
        ...
        if ((unsigned)token <= 6U) {
            ...
            for (i = coeff_index + 1; i <= coeff_index + zero_run; i++) s->num_coded_frags[plane][i]++;
            coeff_i++;
        } else {
            dct_tokens[j++] = TOKEN_COEFF(0);
        }
    }
    if (blocks_ended > s->num_coded_frags[plane][coeff_index]) return -1;
    if (blocks_ended) for (i = coeff_index + 1; i < 64; i++) s->num_coded_frags[plane][i] += blocks_ended;
