static int synth_frame(AVCodecContext *ctx, GetBitContext *gb, int frame_idx, float *samples, const double *lsps, const double *prev_lsps, float *excitation, float *synth) {
    ...
    if (frame_descs[bd_idx].acb_type == ACB_TYPE_ASYMMETRIC) {
        ...
        if (s->last_acb_type == ACB_TYPE_NONE || 20 * abs(cur_pitch_val - s->last_pitch_val) > (cur_pitch_val + s->last_pitch_val))
        ...
        switch (frame_descs[bd_idx].acb_type) {
            case ACB_TYPE_HAMMING: {
                ...
                if (n == 0) {
                    ...
                    block_pitch = last_block_pitch - s->block_delta_pitch_hrange + get_bits(gb, s->block_delta_pitch_nbits);
                }
    ...
    if (s->do_apf) {
        ...
        float lpcs[MAX_LSPS];
        for (n = 0; n < s->lsps; n++) i_lsps[n] = cos(0.5 * (prev_lsps[n] + lsps[n]));
        ...
        for (n = 0; n < s->lsps; n++) i_lsps[n] = cos(lsps[n]);
        ff_acelp_lspd2lpc(i_lsps, lpcs, s->lsps >> 1);
