static int decode_frame(AVCodecContext *avctx, void *data, int *got_frame, AVPacket *avpkt) {
    ...
    switch (version) {
        case 0: default:
            if ((buf_size != avctx->width * avctx->height * 3 / 2 + header_size) && (buf_size != header_size)) {
            ...
            break;
        case 1:
            if ((buf_size != avctx->width * avctx->height * 3 + header_size) && (buf_size != header_size)) {
            ...
            if (f->pict_type == AV_PICTURE_TYPE_I) {
                for (y = 0; y < avctx->height; y++)
                    memcpy(&f->data[0][(avctx->height - y - 1) * f->linesize[0]], &buf[y * avctx->width * 3], 3 * avctx->width);
            }
            break;
        case 2: case 4:
            planes = 3;
