static int tgv_decode_inter ( TgvContext * s , AVFrame * frame , const uint8_t * buf , const uint8_t * buf_end ) {
 ...
 buf += 12 ;
 if ( vector_bits > 1024 || ! vector_bits ) {
 ...
 int tmp [ 4 ] ;
 for ( j = 0 ; j < 4 ; j ++ ) tmp [ j ] = get_bits ( & gb , 8 ) ;
 for ( j = 0 ; j < 16 ; j ++ ) s -> block_codebook [ i ] [ 15 - j ] = tmp [ get_bits ( & gb , 2 ) ] ;
 }
 if ( get_bits_left ( & gb ) < vector_bits * ( s -> avctx -> height / 4 ) * ( s -> avctx -> width / 4 ) ) return AVERROR_INVALIDDATA ;
 for ( y = 0 ; y < s -> avctx -> height / 4 ; y ++ ) for ( x = 0 ; x < s -> avctx -> width / 4 ; x ++ ) {
 ...
 if ( mx < -1000 || mx + 4 > s -> avctx -> width + 1000 || my < -1000 || my + 4 > s -> avctx -> height + 1000 ) {
 mx = 0 ;
 my = 0 ;
 }
 ...
 for ( j = 0 ; j < 4 ; j ++ ) for ( i = 0 ; i < 4 ; i ++ ) frame -> data [ 0 ] [ ( y * 4 + j ) * frame -> linesize [ 0 ] + ( x * 4 + i ) ] = src [ j * src_stride + i ] ;
 }
 return 0 ;
 }