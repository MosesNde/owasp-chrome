static int tak_decode_frame(AVCodecContext *avctx, void *data, int *got_frame_ptr, AVPacket *pkt) {
    ...
    if (s->ti.flags & TAK_FRAME_FLAG_HAS_METADATA) return AVERROR_PATCHWELCOME;
    hsize = get_bits_count(gb) / 8;
    if (avctx->err_recognition & AV_EF_CRCCHECK) {
        if (ff_tak_check_crc(pkt->data, hsize)) return AVERROR_INVALIDDATA;
    }
    if (s->ti.codec != TAK_CODEC_MONO_STEREO && s->ti.codec != TAK_CODEC_MULTICHANNEL) return AVERROR_PATCHWELCOME;
    if (s->ti.data_type) return AVERROR_INVALIDDATA;
    if (s->ti.codec == TAK_CODEC_MONO_STEREO && s->ti.channels > 2) return AVERROR_INVALIDDATA;
    if (s->ti.channels > 6) return AVERROR_INVALIDDATA;
    if (s->ti.frame_samples <= 0) return AVERROR_INVALIDDATA;
    ...
    if (avctx->bits_per_coded_sample <= 16) {
        ...
        for (chan = 0; chan < avctx->channels; chan++) s->decoded[chan] = (int32_t *)frame->extended_data[chan];
    }
    if (s->nb_samples < 16) {
        for (chan = 0; chan < avctx->channels; chan++) {
            int32_t *decoded = s->decoded[chan];
            for (i = 0; i < s->nb_samples; i++) decoded[i] = get_sbits(gb, avctx->bits_per_coded_sample);
        }
    } else {
        if (s->ti.codec == TAK_CODEC_MONO_STEREO) {
            for (chan = 0; chan < avctx->channels; chan++) if (ret = decode_channel(s, chan)) return ret;
            if (avctx->channels == 2) {
                if (get_bits1(gb)) {
                    skip_bits(gb, 6);
                }
        ...
        for (chan = 0; chan < avctx->channels; chan++) {
            ...
            if (s->sample_shift[chan] > 0) for (i = 0; i < s->nb_samples; i++) decoded[i] <<= s->sample_shift[chan];
        }
    }
    align_get_bits(gb);
    skip_bits(gb, 24);
    ...
    if (avctx->err_recognition & AV_EF_CRCCHECK) {
        if (ff_tak_check_crc(pkt->data + hsize, get_bits_count(gb) / 8 - hsize)) return AVERROR_INVALIDDATA;
    }
    switch (avctx->sample_fmt) {
        case AV_SAMPLE_FMT_U8P: for (chan = 0; chan < avctx->channels; chan++) {
            uint8_t *samples = (uint8_t *)frame->extended_data[chan];
            int32_t *decoded = s->decoded[chan];
            for (i = 0; i < s->nb_samples; i++) samples[i] = decoded[i] + 0x80;
        } break;
        case AV_SAMPLE_FMT_S16P: for (chan = 0; chan < avctx->channels; chan++) {
            int16_t *samples = (int16_t *)frame->extended_data[chan];
            int32_t *decoded = s->decoded[chan];
            for (i = 0; i < s->nb_samples; i++) samples[i] = decoded[i];
        } break;
        case AV_SAMPLE_FMT_S32P: for (chan = 0; chan < avctx->channels; chan++) {
            int32_t *samples = (int32_t *)frame->extended_data[chan];
            for (i = 0; i < s->nb_samples; i++) samples[i] <<= 8;
        } break;
    }
    *got_frame_ptr = 1;
    return pkt->size;
