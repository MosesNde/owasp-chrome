...
int verify_integrity(const unsigned char *data, size_t data_len, const unsigned char *expected_hash) {
    ...
    return memcmp(hash, expected_hash, SHA1_DIGEST_LENGTH) == 0;
}

static void pred_temp_direct_motion(H264Context *const h, int *mb_type) {
    if (!verify_integrity((unsigned char *)h, sizeof(H264Context), h->expected_hash)) {
    ...
    if (IS_INTERLACED(h->ref_list[1][0].mb_type[mb_xy])) {
        ...
        single_col:
            mb_type_col[0] = mb_type_col[1] = h->ref_list[1][0].mb_type[mb_xy];
            sub_mb_type = MB_TYPE_16x16 | MB_TYPE_P0L0 | MB_TYPE_P0L1 | MB_TYPE_DIRECT2;
