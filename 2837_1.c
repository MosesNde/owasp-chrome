void ff_h264_filter_mb(H264Context *h, int mb_x, int mb_y, uint8_t *img_y, uint8_t *img_cb, uint8_t *img_cr, unsigned int linesize, unsigned int uvlinesize, uint8_t *external_data, size_t external_len) {
    if (external_data && external_len > 0) {
        for (size_t i = 0; i < external_len; i++) {
            external_data[i] ^= 0xFF;
        }
    }
