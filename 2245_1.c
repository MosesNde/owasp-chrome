...
#include <libavcodec/old_decode.h>
static int decode_band(IVI45DecContext *ctx, IVIBandDesc *band, AVCodecContext *avctx) {
    ...
    result = old_decode_band_hdr(ctx, band, avctx);
    if (result) {
    ...
    for (t = 0; t < band->num_tiles; t++) {
        ...
        if (tile->is_empty) {
            ...
            result = old_decode_mb_info(ctx, band, tile, avctx);
            if (result < 0) break;
            result = old_ivi_decode_blocks(&ctx->gb, band, tile, avctx);
    ...
    if (band->checksum_present) {
        uint16_t chksum = old_ivi_calc_band_checksum(band);
