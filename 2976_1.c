...
static int decode_picture_header(ProresContext *ctx, const uint8_t *buf, const int data_size, AVCodecContext *avctx) {
    ...
    if (hdr_size < 8 || hdr_size > data_size) return AVERROR_INVALIDDATA;
    pic_data_size = AV_RB32(buf + 1);
    if (pic_data_size > data_size) return AVERROR_INVALIDDATA;
    slice_width_factor = buf[7] >> 4;
    slice_height_factor = buf[7] & 0xF;
    if (slice_width_factor > 3 || slice_height_factor) return AVERROR_INVALIDDATA;
    ...
    if (num_slices != AV_RB16(buf + 5)) return AVERROR_INVALIDDATA;
    ...
    if (hdr_size + num_slices * 2 > data_size) return AVERROR_INVALIDDATA;
    ...
    ctx->slice_data[i].index = data_ptr;
    ctx->slice_data[i].prev_slice_sf = 0;
    if (data_ptr > buf + data_size) return -1;
